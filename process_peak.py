...
def process_peak(experimental_dict, i, df_wide, decimals=2):
    ...
    peak_start_index, peak_end_index, background_start_index, background_end_index = load_peak(experimental_dict, i, df_wide)
    background_value = background(background_start_index, background_end_index, df_wide)
    df_peak_processed = subtract_background(df_wide, peak_start_index, peak_end_index, background_value, decimals)
    peak_conc, peak_area = calculate_peak_area_and_conc(df_peak_processed, decimals)
    measurements, slope, y_int, rsq = calculate_decay(df_peak_processed)
    
    return peak_conc, peak_area, measurements, slope, y_int, rsq